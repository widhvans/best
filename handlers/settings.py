...
from database.db import (
    ...
    get_file_by_unique_id # <-- Import get_file_by_unique_id
)
from utils.helpers import ( # <-- Removed 'create_post' from imports
    go_back_button, get_main_menu, clean_filename, 
    calculate_title_similarity, notify_and_remove_invalid_channel
)
from features.poster import get_poster # <-- Import get_poster
...
# ... (The first part of the file with menu parts and basic handlers remains the same) ...
...
# --- MODIFIED: start_backup_process now uses the new posting logic ---
...
async def start_backup_process(client, query):
    ...
    target_channel_id = int(query.data.split("_")[-1])
    ...
    try:
        ...
        for i, file_docs_batch in enumerate(batches):
            ...
            try:
                ...
                if user_settings.get('show_poster', True):
                    first_filename = file_docs_batch[0]['file_name']
                    primary_title, year = clean_filename(first_filename)
                    poster = await get_poster(primary_title, year)
                    if poster:
                        await client.send_photo(target_channel_id, photo=poster, caption=f"🎬 **{primary_title}**")

                # File Sending Logic
                for doc in file_docs_batch:
                    # Get the original message to copy it
                    source_chat_id = int("-100" + doc['raw_link'].split('/')[-2])
                    message_id = int(doc['raw_link'].split('/')[-1])
                    original_message = await client.get_messages(source_chat_id, message_id)

                    # Create vertical buttons
                    get_link = f"http://{client.vps_ip}:{client.vps_port}/get/{doc['file_unique_id']}"
                    watch_link = f"http://{client.vps_ip}:{client.vps_port}/watch/{doc['stream_id']}"
                    ...
                    # Add footer buttons
                    user_footer_buttons = user_settings.get('footer_buttons', [])
